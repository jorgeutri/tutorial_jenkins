...
public final class LanguageDetectorImpl implements LanguageDetector {
	...
	/*Añadida para ver la probabilidad de idioma*/
	private double verProb;
	/**
	 * Use the {@link LanguageDetectorBuilder}.
	 */
	LanguageDetectorImpl(@NotNull NgramFrequencyData ngramFrequencyData, double alpha, Optional<Long> seed,
			int shortTextAlgorithm, double prefixFactor, double suffixFactor, double probabilityThreshold,
		...
		if (probabilityThreshold < 0d || probabilityThreshold > 1d)
			throw new IllegalArgumentException(
					"probabilityThreshold must be between 0 and 1, but was: " + probabilityThreshold);
		if (minimalConfidence < 0d || minimalConfidence > 1d)
			throw new IllegalArgumentException(
					"minimalConfidence must be between 0 and 1, but was: " + minimalConfidence);
		...
		this.priorMap = (langWeightingMap == null) ? null
				: Util.makeInternalPrioMap(langWeightingMap, ngramFrequencyData.getLanguageList());
		this.ngramExtractor = ngramExtractor;
	}

	@Override
	public Optional<LdLocale> detect(CharSequence text, double alpha) {
		List<DetectedLanguage> probabilities = getProbabilities(text);
		if (probabilities.isEmpty()) {
			return Optional.absent();
		} else {
			DetectedLanguage best = probabilities.get(0);
			this.verProb =  probabilities.get(0).getProbability();
			if (best.getProbability() >= alpha) {
	...
	private boolean updateLangProb(@NotNull double[] prob, @NotNull String ngram, int count, double alpha) {
		...
		if (logger.isTraceEnabled())
			logger.trace(ngram + "(" + Util.unicodeEncode(ngram) + "):"
